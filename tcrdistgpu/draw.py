...
def draw_categories(
    ...
    savefig="test.pdf",
    edge_palette='Set2',
    get_colors_only = False, # use if you just want to know colors
    **kwargs
):
    if remove_self_edges:
        G.remove_edges_from(nx.selfloop_edges(G))
    ...
    # Color fill (node color)
    node_to_color, key_to_color = get_node_to_color_map(
        data=data,
        key_to_color = key_to_color_override,
        null_value = null_value,
        null_color = null_color,
        col=color_col,
        palette=palette,
        palette_list=pallete_list
    )
    # if key_to_color_override is not None:
    #     node_to_color = {k: null_color if k == null_value else v for k, v in node_to_color.items()}
    # if node_to_color_override is not None:
    #     node_to_color = node_to_color_override
    #     key_to_color  = key_to_color_override
    ...
    if get_colors_only:
        return key_to_color
    # Optional: node border (edge color)
    if edge_color_col is not None:
        if edge_key_to_color is not None:
            node_to_edgecolor = data[edge_color_col].map(edge_key_to_color).to_dict()
        else:
    ...
    # Node sizes
    if size_col:
        size_series = data[size_col] * size_scale
        size_series = size_series.clip(*size_clip)
        node_size_map = size_series.to_dict()
        node_sizes = [node_size_map.get(n, size_clip[0]) for n in G.nodes]
    else:
...
def get_node_to_color_map(data, key_to_color = None, 
        null_value = None,
        null_color = None,
        col = 'epitope',
        palette = 'tab20', 
